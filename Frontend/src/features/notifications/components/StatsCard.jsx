...
export default function StatsCard({ title, value, subtitle, icon, color = 'purple' }) {
  const IconComponent = Icons[icon] || Icons.HelpCircle;
  const style = colorStyles[color] || colorStyles.purple;

  return (
    <div className="bg-white border border-[#E5E7EB] rounded-2xl p-5 shadow-sm hover:-translate-y-1 hover:shadow-md transition-all duration-300 flex items-center gap-4 select-none w-full">
